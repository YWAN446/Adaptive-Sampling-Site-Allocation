...
# siteInit <- function(n,n.site,sen.site,random=FALSE){
#   if (random==FALSE){
#     return(sample(1:n.site,prob = sen.site,n))
#   } else {
#     return(sample(1:n.site,n))
#   }
# }
...
siteAnalysis <- function(samples, mat.conc, uf.pos){
...
siteDrop <- function(psu.res,n.drop,alpha=0.1){ #alpha is the weight for total number of positive
  k.psu <- psu.res[,1]
  obs <- psu.res[,-c(1,2)]
  ...
  eff.site <- infoLoss + alpha * totalPos
