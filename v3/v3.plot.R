...
#outbreak
pdf(file=paste0("./",version,"/output/res_outbreak",lambda,"_",Sys.Date(),".pdf"),width=14,height=10)
plot(sewage.lines.df, lwd = sqrt(iterative.ssn@data$addfunccol)*5, col = "black", xlab="x-coordinate", ylab="y-coordinate", 
     main = paste("Peak number of new shedders",lambda,"per day"), asp=0.25, ylim=rev(sewage.lines.df@bbox[2,]), cex.main=2)
points(iterative.ssn@obspoints@SSNPoints[[1]]@point.coords[,1], iterative.ssn@obspoints@SSNPoints[[1]]@point.coords[,2],cex=latrine_range$outbreak.length/10)
legend( x="bottomleft", bty='n', title="Probability of having shedders",
        legend=paste(as.character(round(quantile(latrine_range$outbreak.length/n.latrine,probs = c(1,0.95,0.75,0.5,0.25))*100,3)),"% - ",c("max","95th percentile","75th percentile","medium","25th percentile")),
        col="black", cex=1, pch=1, pt.cex=quantile(latrine_range$outbreak.length,probs = c(1,0.95,0.75,0.5,0.25))/10)
dev.off()