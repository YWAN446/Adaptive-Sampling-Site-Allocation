n.infected <- rpois(n.days,getLambda(1:n.days,lambda,season = TRUE))

#simulate which latrines the shedders will go;
mat.latrine.length <-matrix(0,nrow=n.days,ncol=n.latrine)
#unequal length
...
for (t in 1:n.days){
  #insert outbreak in geographic locations;
  if ((t-1) %% outbreak.days == 0){
    outbreak.x <- runif(1,gps.points.range[1,1],gps.points.range[1,2])
    outbreak.y <- runif(1,gps.points.range[2,1],gps.points.range[2,2])
    outbreak.points <- which(((gps.points[,1]-outbreak.x)/gps.points.range.x)^2+((gps.points[,2]-outbreak.y)/gps.points.range.y)^2<=outbreak.range)
  ...
  mat.latrine.length[t,] <- latrine_range$outbreak.length
  if (n.infected[t]>0){
    vec.loc.pos <- runif(n.infected[t],0,n.latrine)
    for (k in sapply(vec.loc.pos,FUN = function(x){which(x>=latrine_range$outbreak.start & x<latrine_range$outbreak.end)})){
